...
#cur.execute("DROP TABLE IF EXISTS crnjakt_workouts")
#cur.execute("DROP TABLE IF EXISTS crnjakt_rowers")
...
def insert_workout(conn, cursor, workouts):
        ...
        for rower_id in rower_ids:
            ...
            cursor.execute('SELECT partner_id FROM crnjakt_rowers WHERE partner_id = %s', (rower_id,))
...
def get_latest_workout_date(cursor, partner_id):
    ...
    sql = "SELECT MAX(date) as latest_date FROM crnjakt_workouts WHERE rower_id = %s"
...
for rower in rowers.items():
    insert_rower(conn, cur, rower[1])

    try:
        rower_id = rower[1]['partner_id']
        latest_date = get_latest_workout_date(cur,rower_id)
        ...
        for workout_id in data['data']:
            ...
            try:
                ...
                if len(workouts) ==1:
                    insert_workout(conn, cur, workouts)
                    workouts=[]
